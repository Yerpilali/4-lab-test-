...
def create_book(db: Session, book: schemas.BookCreate, publishings_id: int):

    db_book = models.Book(**book.dict(), publishings_id=publishings_id)
    db.add(db_book)
    db.commit()
    db.refresh(db_book)
    return db_book
...
def get_givings_by_reader_id(db: Session, reader_id: int):
    return db.query(models.Giving).filter(models.Giving.readers_id == reader_id).all()


def get_givings_by_book_id(db: Session, book_id: int):
    return db.query(models.Giving).filter(models.Giving.books_id == book_id).all()
